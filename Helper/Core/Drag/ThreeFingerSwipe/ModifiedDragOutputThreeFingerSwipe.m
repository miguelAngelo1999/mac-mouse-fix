...
/// On macOS 27+ (and any future version), the WindowServer drops synthetic dockSwipe CGEvents.
/// We detect this at startup by checking the OS version once and caching the result.
/// macOS 26 (Tahoe 26.x) still works with the smooth gesture path.
/// macOS 27+ requires the SymbolicHotKeys fallback.
static BOOL sUseSymbolicHotKeyFallback = NO;

+ (void)load_Manual {
    NSOperatingSystemVersion ver = NSProcessInfo.processInfo.operatingSystemVersion;
    sUseSymbolicHotKeyFallback = (ver.majorVersion >= 27);
}

static BOOL useSymbolicHotKeyFallback(void) {
    return sUseSymbolicHotKeyFallback;
}
